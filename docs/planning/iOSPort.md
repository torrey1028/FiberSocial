...
| New-event polling | WorkManager periodic (user-picks a qualitative cadence — Hourly/A few times a day/Once a day — mapped to a 1/6/24h interval, network-constrained) | `BGAppRefreshTask` | **The big behavioral gap.** iOS background refresh is opportunistic: the OS decides cadence from usage patterns; `earliestBeginDate` is a floor, not a schedule, and force-quit apps don't refresh. Mitigate: qualitative cadence labels (phase 0f, landed) so the setting promises nothing iOS can't keep, plus a sync on every foreground activation (cheap, already idempotent). |
