# Play Store screenshots

Phone screenshots submitted with the Google Play closed-testing listing.
Captured from a physical device, dark theme.

| File | Screen |
|------|--------|
| `01-group-feed.png` | Group feed — forum topic list (FiberSocial App Support). |
| `02-group-drawer.png` | Group drawer — "Your Groups" list. |
| `03-group-feed.png` | Group feed, same screen as `01` a moment later. |
| `04-event-detail.png` | Event detail screen. |

Play Console accepted phone-only screenshots for the closed test — no
tablet-specific set was required.

# iOS screenshots

The same four screens, replicated on iOS for comparison. Captured from the
iPhone 14 Plus Simulator (iOS 26.3) at 1284×2778 — App Store Connect's
required 6.7" display size — dark theme, same Ravelry account.

| File | Screen |
|------|--------|
| `ios-01-group-feed.png` | Group feed — forum topic list (FiberSocial App Support). |
| `ios-02-group-drawer.png` | Group drawer — "Your Groups" list. |
| `ios-03-group-feed.png` | Group feed, same screen as `ios-01` a moment later. |
| `ios-04-event-detail.png` | Event detail screen — "Fake Event" test event, found in "FiberSocial App Test Group - super long name on purpose" rather than "FiberSocial App Support" (its group assignment on Ravelry differs from when the Android set was captured). |
