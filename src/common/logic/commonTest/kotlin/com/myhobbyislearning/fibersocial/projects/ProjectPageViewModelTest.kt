...
import com.myhobbyislearning.fibersocial.feed.nullMessageApiClient
    ...
    fun `session expiry from an open dropped after dismissal does not signal expiry`() = runTest(UnconfinedTestDispatcher()) {
        ...
        val vm = ProjectPageViewModel(
            suspendableRoutingApiClient { _ ->
                gate.await()
                throw com.myhobbyislearning.fibersocial.auth.SessionExpiredException("expired")
            },
            this,
        )
        vm.open(link)
        ...
        val vm = ProjectPageViewModel(
            routingApiClient(route = { path ->
                when {
                    ...
                    path == "/comments/1.json" ->
                        throw com.myhobbyislearning.fibersocial.auth.ForbiddenException("nope")
                    else -> """{"project":{"id":7,"name":"Autumn Socks"}}"""
                }
            }),
            this,
        ...
        val vm = ProjectPageViewModel(
            routingApiClient(route = { path ->
                when {
                    path.endsWith("/comments.json") -> """{"comments":[]}"""
                    path == "/comments/create.json" ->
                        throw com.myhobbyislearning.fibersocial.auth.ForbiddenException("no scope")
                    else -> """{"project":{"id":7,"name":"Autumn Socks"}}"""
                }
            }),
            this,
