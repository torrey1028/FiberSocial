...
- Pushing a `v*.*.*` tag triggers `.github/workflows/release.yml`, which builds the signed release APK and publishes it as a GitHub Release (title = the tag, notes auto-generated from merged PRs since the previous tag via `gh release create --generate-notes`), marked as the repo's latest release.
